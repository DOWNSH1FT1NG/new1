...
bool ArrangeWindow::keyPressed(const KeyPress& key, Component* originatingComponent)
{
    ...
    else if (key.getTextCharacter() == 114) // r
    {
        if (sequenceRegions.add(addRegions()) != nullptr)
        {
            if (regionCreated == true)
            {
                ...
                regionData.add(new RegionData);
                addRegionData(); // если он не сам добавляет

                auto* newRegion = regionData.getLast(); // безопасный доступ

                listener->resizeViewPorts(newRegion->yPosition);
                listener->setPianoRollSize(createRegionCounter, sequenceRegions[createRegionCounter]->getWidth());
                setPianoRollBoundary(((newRegion->xSize / 80) * 224) + 14, createRegionCounter);

                for (int count = 0; count < createRegionCounter; count++)
                    sequenceRegions[count]->setRegionNumber(count);

                createRegionCounter++; // <- только после!
                regionCreated = false;
            }
        }
        return true;
